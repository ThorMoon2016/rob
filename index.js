...
$(document).ready(function () {
  function animateLightRay() {
    $(".start-order-box").each(function () {
      $(this).find("::before").animate(
        ...
        },
        {
          ...
          step: function (now, fx) {
            // Rotacija kruga
            $(this).css("transform", "translate(-50%, -50%) rotate(" + now * 0.2 + "deg)");
            // Boja kruga
            var opacity = now * 0.002;
            $(this).css("background-color", "rgba(53, 221, 67, " + opacity + ")");
          },
        }
      );
    });
  ...
  setInterval(animateLightRay, 5000); // Ponavljanje animacije svakih 5 sekundi
});
